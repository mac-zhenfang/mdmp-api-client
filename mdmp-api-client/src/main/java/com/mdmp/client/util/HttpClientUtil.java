...
import com.mdmp.client.RequestResponse;
...
//      }
      ...
      if (header.size() > 0) {
        ...
        for (Map.Entry<String, String> entry : header.entrySet()) {
          if (!entry.getKey().equals(APIConstants.SQL)) {
...
//    method.addHeader(APIConstants.APP_TOKEN, AppTokenUtil.makeToken(appName));
  ...
  /**
   * this method is  for http GET request without customized logic related with RAMP
   */
  public static RequestResponse requestGet(String url, Map<String, String> httHeader) throws IOException {
      HttpGet method = new HttpGet(url);
      for (Map.Entry<String, String> entry : httHeader.entrySet()) {
          method.addHeader(entry.getKey(), entry.getValue());
      }
      //populateParameterAndContent(method, header, null);
      RequestResponse rr = new RequestResponse();
      HttpClient client = HttpClientUtil.getDefaultHttpClient();
      HttpResponse response = client.execute(method);
